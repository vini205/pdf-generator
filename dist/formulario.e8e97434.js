/* Settings*/ let tipoManutencao = getElement("itipo");
campoCorretiva();
...
*/ function addMaterial() {
    ...
    valorUniatario.placeholder = "Valor Unit\xe1rio";
    ...
    descricao.placeholder = "Decri\xe7\xe3o";
...
function manutencaoPreventiva() {
    ...
    for (const button of p)if (button.nodeName == "BUTTON") {
        if (button.classList.contains("btn__prev--value")) //Botões com parâmetros
...
function addMaterials() {
    if (materialHolder.children.length - 1 > 0) {
        ...
        offToggle([], [
            document.querySelector(".materiais")
        ]);
        ...
        for(let i = 1; i < materialsNumber + 1; i++){
            ...
            let quantidade = material.children[0].value, valorUnitario = material.children[1].value, descricao = material.children[2].value, unidade = material.children[3].value;
            let total = valorUnitario * quantidade;
            sum += Number(total);
            createMaterialField(descricao, quantidade, unidade, "R$ " + valorUnitario, "R$ " + total);
        }
...
function resetTable() {
    ...
    try {
        ...
        for(let i = c; 0 < c - 1; i--){
            ...
            if (e.classList.contains("titulo") || e.id == "total") continue;
            e.remove();
        }
    } catch (error) {}
//Para evitar mostrar erro no index 
}
function createMaterialField(descricao = "descri\xe7\xe3o", quantidade = "0", unidade = "0", valorUnitario = "0", sum) {
    // Cria os campos para armazenar os dados, 
    // e os adiciona na tabela
    let desc = document.createElement("div"), quant = document.createElement("div"), uni = document.createElement("div"), valorUni = document.createElement("div"), total = document.createElement("div");
    desc.innerHTML = descricao;
    quant.innerHTML = quantidade;
    uni.innerHTML = unidade;
    valorUni.innerHTML = valorUnitario;
    total.innerHTML = sum;
    document.querySelector("#materialTotal").before(desc, quant, uni, valorUni, total);
}
