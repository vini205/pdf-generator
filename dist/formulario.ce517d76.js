...
function putInInput(data, input) {
    for (const i of input)i.value = data;
}
...
function add() {
    ...
    for(let i = 0; i < topass.length; i += 2){
        ...
        if (topass[i + 1].tagName == "TEXTAREA") addLines(topass[i + 1]);
        tableDefault.push([
            THORATOTAL,
            THORATOTAL.innerHTML
        ]);
        let a1 = document.getElementsByClassName("table")[0];
...
function addTOTable(data, where) {
    if (data.tagName == "TEXTAREA") {
        ...
        if (where.innerHTML == "/2023") where.innerHTML = `${date}`;
        else where.innerHTML += `: ${date}`;
...
function resetTable(x) {
    for(let i = 0; i < x.length; i++){
        const e = x[i];
        if (e.tagNAme == "TEXTAREA") e[0].value = e[1];
        else e[0].innerHTML = e[1];
    }
...
function createMaterialBtns(n) {
    ...
    p.addEventListener("click", ()=>{
        if (holder.children[1] === undefined) holder.append(IMATERIALNUMBER, pCorfim);
        div.remove();
    });
...
function addMaterialToTable() {
    ...
    for(let i = 0; i < materiais.length; i++){
        ...
        for(let td = 0; td < element.cells.length - 1; td++){
            ...
            if (td + 2 === element.cells.length) element.cells[4].innerHTML = valueTotal;
            if (values[td].type == "number" && m !== "imqtd") {
                if (values[td].value === "") e.innerHTML = "R$ 0";
                else e.innerHTML = `R$ ${value}`;
...
function updateImgPreview() {
    ...
    if (images.length === 0) return "Sem imagens";
        ...
        for (const image of images){
            ...
            imageText[imgName] = [
                img,
                ""
            ]; //Guarda o elemento para apagá-lo dps
        }
...
function verifyMaterial() {
    ...
    if (n >= 1) {
        ...
        for (const input of i)holder.appendChild(input);
    }
}

//# sourceMappingURL=formulario.ce517d76.js.map
